...
export default function FindMatchesScreen() {
  ...
  const buildStartupRequestPayload = (
    startup: Startup,
    investors: Investor[]
  ) => ({
    startup: {
  ...
  const buildInvestorRequestPayload = (
    investor: Investor,
    startups: Startup[]
  ) => ({
    investor: {
  ...
  const findMatches = async () => {
    ...
    try {
      ...
      if (isStartup(userProfile)) {
        ...
        if (investors.length === 0) {
          toast({
            ...
            description:
              "No investors are currently registered in the platform.",
            variant: "destructive",
          });
          setLoading(false);
          return;
      ...
      if (Array.isArray(data.matches)) {
        ...
        toast({
          title: `${entityType === "investor" ? "Investor" : "Startup"
            } Matches Found`,
          description: `Found ${data.matches.length} potential ${entityType}${data.matches.length === 1 ? "" : "s"
            } for you.`,
        });
      } else {
        toast({
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                {isUserStartup ? (
                  ...
                  <>
                    ...
                    {(userProfile as Investor).preferredIndustries &&
                      (userProfile as Investor).preferredIndustries!.length >
                      0 && (
                        <div className="flex items-center space-x-2">
                          <Briefcase className="h-4 w-4 text-gray-500" />
                          <span className="text-sm text-gray-700">
                            {(userProfile as Investor).preferredIndustries!.slice(
                              0,
                              2
                            ).join(", ")}
                          </span>
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-gray-600 text-center max-w-md">
              Our AI is analyzing {targetEntityLower} profiles and matching them
              with your {isUserStartup ? "startup's" : "investment"} unique
              profile...
            </p>
          </div>
        )}
        ...
        {!loading && matches.length === 0 && userProfile && (
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">
              {cachedMatches.length > 0
                ? "Cache Cleared"
                : `Ready to Find ${targetEntity}?`}
            </h3>
            <p className="text-gray-600 max-w-md mx-auto">
              {cachedMatches.length > 0
                ? `Your cached matches have been cleared. Click 'Find New ${targetEntity}' to search again.`
                : `Click the 'Find New ${targetEntity}' button above to discover ${targetEntityLower} that align with your ${isUserStartup
                  ? "startup's goals and funding requirements"
        ...
        {matches.length > 0 && (
          <div className="mb-6">
            ...
            <p className="text-gray-600 mt-1">
              Based on your {isUserStartup ? "startup" : "investment"} profile,
              here are {targetEntityLower} most likely to be{" "}
              {isUserStartup
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {matches.map((match, idx) => (
            ...
            >
              {/* Match Score Header */}
              <div
                className={`${getMatchScoreColor(match.match_score)} p-4 text-white`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
              ...
              <CardHeader className="pb-3">
                ...
                <Badge variant="outline" className="w-fit text-xs">
                  ID:{" "}
                  {isStartupMatch(match)
                    ? match.startup_id
                    : (match as InvestorMatch).investor_id}
                </Badge>
              </CardHeader>
